...
parser.add_argument('-t', action='store_const', const=0.5)
...
# 没有传入需要忽略的类型 设置忽略类型为None
if args.ignore is None:
    args.ignore = []

# 默认不传入单独设置iou, specific_iou_flagged为False
...
# 默认不播放比对结果动画, 如果要比对结果动画需要导入OpenCV
...
# 默认不绘制图片, 用户设置需要绘制就导入 matplotlib.pyplot
draw_plot = False
if not args.no_plot:
    try:
...
def log_average_miss_rate(prec, rec, num_images):
    """
        ...
        output:
                ...
                mr | miss rate  漏报率
                fppi | false positives per image   每张图片误报
...
with open(output_files_path + "/output.txt", 'a') as output_file:
    ...
    for class_name in sorted(dr_classes):
        ...
        text += "perssion=" + str(round(int(count_true_positives[class_name]) / int(n_det), 4)) + "  "
        text += "recall=" + str(round(int(count_true_positives[class_name]) / int(gt_counter_per_class[class_name]), 4)) + "\n"
